...
function formatStatLine(star: LandingThreeStar): string {
  if (star.position === "G") {
    ...
    const svPct = star.savePctg != null ? (star.savePctg * 100).toFixed(1) + "%" : "—";
      ...
      <div className={styles["three-stars-row"]}>
        {stars.map((star) => (
          <div
            ...
            style={{ "--star-team-color": getTeamPrimaryColor(star.teamAbbrev) } as React.CSSProperties}
          >
            ...
            <div className={styles["three-star-card__info"]}>
              <span className={styles["three-star-card__name"]}>{star.name.default}</span>
              <span className={styles["three-star-card__meta"]}>
                {star.teamAbbrev} · {star.position}
              </span>
              <span className={styles["three-star-card__stats"]}>{formatStatLine(star)}</span>
              <span className={styles["three-star-card__label"]}>{STAR_LABELS[star.star - 1]}</span>
            </div>
          </div>
        ))}
      </div>
