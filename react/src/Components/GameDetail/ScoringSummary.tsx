        ...
        {periodsWithGoals.map((period) => (
          ...
          >
            ...
            {period.goals.map((goal, i) => {
              ...
              const assists = goal.assists.map((a) => a.name.default).join(", ");
              const scoreDisplay = `${goal.awayScore}–${goal.homeScore}`;
              const goalTeamColor = getTeamPrimaryColor(goal.teamAbbrev.default);

              return (
                <div
                  key={i}
                  className={styles["scoring-goal-row"]}
                  style={{ "--goal-team-color": goalTeamColor } as React.CSSProperties}
                >
                  <img
                  ...
                  <div className={styles["scoring-goal__info"]}>
                    <span className={styles["scoring-goal__scorer"]}>{goal.name.default}</span>
                    {assists && (
                      <span className={styles["scoring-goal__assists"]}>A: {assists}</span>
                    )}
                  </div>
                  <div className={styles["scoring-goal__right"]}>
                    <span className={styles["scoring-goal__score"]}>{scoreDisplay}</span>
                    <span className={styles["scoring-goal__time"]}>{goal.timeInPeriod}</span>
                    {badge && (
                      <span className={`${styles["scoring-badge"]} ${styles[`scoring-badge--${badge.toLowerCase()}`]}`}>
                        {badge}
                      </span>
                    )}
                  </div>
