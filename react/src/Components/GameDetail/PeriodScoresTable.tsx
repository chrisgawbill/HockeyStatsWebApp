...
function PeriodScoresTable({ periodScores, homeAbbrev, awayAbbrev }: PeriodScoresTableProps) {
  ...
  return (
    <section className={`${styles["game-detail-section"]} ${shared.section}`}>
      ...
      <div className={`${styles["period-scores-wrapper"]} ${shared.surface} ${shared.surfaceScroll}`}>
        <table className={styles["period-scores-table"]}>
          ...
          <tbody>
            <tr style={{ "--period-team-color": awayColor } as React.CSSProperties}>
              <td className={styles["period-scores-table__team-col"]}>{awayAbbrev}</td>
              {periodScores.map((p) => (
                <td key={p.periodNum}>{p.awayGoals}</td>
              ))}
              <td className={styles["total-col"]}>{awayTotal}</td>
            </tr>
            <tr style={{ "--period-team-color": homeColor } as React.CSSProperties}>
              <td className={styles["period-scores-table__team-col"]}>{homeAbbrev}</td>
              {periodScores.map((p) => (
