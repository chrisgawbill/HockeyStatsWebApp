...
import styles from "../style/GameDetailPage.module.css";
 ...
 function getPeriodScores(scoring: LandingScoringPeriod[], homeAbbrev: string, awayAbbrev: string) {
   return scoring.map(period => ({
     ...
     homeGoals: period.goals.filter(g => g.teamAbbrev.default === homeAbbrev).length,
     awayGoals: period.goals.filter(g => g.teamAbbrev.default === awayAbbrev).length,
   }));
 }
 ...
 const GameDetailPage = () => {
    ...
    const periodScores = useMemo(() => {
        ...
        return getPeriodScores(landing.summary.scoring, boxscore.homeTeam.abbrev, boxscore.awayTeam.abbrev);
    }, [landing, boxscore]);

    const homeTotals = useMemo(() => {
        if (!boxscore?.playerByGameStats) return null;
        return computeTeamTotals(boxscore.playerByGameStats.homeTeam, boxscore.homeTeam.sog);
    }, [boxscore]);

    const awayTotals = useMemo(() => {
        if (!boxscore?.playerByGameStats) return null;
        return computeTeamTotals(boxscore.playerByGameStats.awayTeam, boxscore.awayTeam.sog);
    }, [boxscore]);
            ...
            {!isFuture && (
                <div className={styles["game-detail-page__content"]}>
                    ...
                    {((landing?.summary?.threeStars?.length ?? 0) > 0) && 
                        <ThreeStars 
                            stars={landing!.summary.threeStars}
                        /> 
                    }
