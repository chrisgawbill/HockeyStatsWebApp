...
import { CLINCH_STATUS_META, ClinchStatus } from '@/features/standings/utils/clinchStatus';

type SortKey = 'PLACE' | 'TEAM' | 'RECORD' | 'POINTS' | 'POINTSPERCENTAGE';

interface Props { standingsData: StandingsTeam[]; standingFormat: string }

export default function LandingPageStandingsTable({ standingsData, standingFormat }: Props) {
  const navigate = useNavigate();
  const location = useLocation();
  const isDivision = standingFormat === 'Division';
  const [isMobile, setIsMobile] = React.useState(() => typeof window !== 'undefined' && window.matchMedia('(max-width: 767px)').matches);
  const [sort, setSort] = React.useState<{ key: SortKey; direction: 'ascending' | 'descending' } | null>(null);
  ...
  const goToTeam = (team: StandingsTeam) => navigate(`/team/${team.id}`, { state: { sourcePath: standingsSourcePath, fallbackPath: standingsSourcePath } });
  ...
  const getClinchStatus = (item: StandingsTeam): ClinchStatus | null => {
    ...
    if (indicator === 'w' || (indicator === 'x' && item.wildCardRank > 0 && item.wildCardRank <= 2)) return 'wildcard';
  ...
  const renderClinchBadge = (status: ClinchStatus | null) => {
    ...
    return <span className={`${styles['standings-clinch-badge']} ${meta.className}`} aria-label={meta.label} title={meta.label}>{meta.badge}</span>;
  };
  ...
  const sortedStandings = [...standingsData].sort((a, b) => {
    ...
    let result = 0;
    switch (sort.key) {
      case 'PLACE': result = (isDivision ? a.divisionStandingsPlace : a.conferenceStandingsPlace) - (isDivision ? b.divisionStandingsPlace : b.conferenceStandingsPlace); break;
      case 'TEAM': result = a.teamName.localeCompare(b.teamName); break;
      case 'RECORD': result = a.wins - b.wins; break;
      case 'POINTS': result = a.points - b.points; break;
      case 'POINTSPERCENTAGE': result = a.pointsPercentage - b.pointsPercentage; break;
    }
  ...
  const headers: [string, SortKey][] = [['#', 'PLACE'], ['Team', 'TEAM'], ['Record', 'RECORD'], ['Pts', 'POINTS']];
  if (!isMobile) headers.push(['P%', 'POINTSPERCENTAGE']);

  return (
    <div className={`${styles['standings-table-shell']} ds-table-wrap`} style={{ height: isDivision ? '20rem' : '38.75rem', marginTop: '1%', marginBottom: '2%' }}>
      <table className={`${styles['standings-table']} ds-table`}>
        <thead><tr>{headers.map(([label, key]) => <th key={key} scope="col">{isMobile ? label : <button type="button" className="ds-table-sort-button" aria-label={`Sort by ${label}`} aria-sort={sort?.key === key ? sort.direction : 'none'} onClick={() => toggleSort(key)}>{label}</button>}</th>)}</tr></thead>
        <tbody>{sortedStandings.map((team) => {
