...
export default function StatsLeaderModal({
  ...
  return (
    <div>
      <AppDialog open={showModal} onClose={handleModalClose} labelledBy={titleId} className={styles['stat-leader-modal']}>
        ...
        <div className={styles['stat-leader-modal__body']}>
          <div className={`${styles['stat-leader-modal__table']} ds-table-wrap`}>
            <table className="ds-table">
              <thead><tr><th scope="col">Name</th><th scope="col">Team</th><th scope="col">Pos</th><th scope="col">{modalTitle}</th></tr></thead>
              <tbody>{statsLeaderData.map((item) => <tr key={`${item.firstName}-${item.lastName}-${item.teamName}`}>
                <td>{item.firstName} {item.lastName}</td>
                <td><span className={styles['stat-leader-modal__team-cell']}><img className={styles['stat-leader-modal__team-logo']} src={item.teamLogo} alt={item.teamName} /><p>{item.teamName}</p></span></td>
                <td>{item.position}</td>
