...
- `api/app.js` requires `./routes/users`, but `api/routes/users.js` is not present in the current tree. Starting the backend may fail until that require or route is fixed.
- `api/utils/seasonHelper.js` uses `export function` syntax while the backend otherwise uses CommonJS `require`. If Node cannot load that file in the current runtime, convert it to CommonJS or configure module support consistently.
