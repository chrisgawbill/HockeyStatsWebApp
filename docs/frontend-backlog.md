...
Two icon dialects, neither Material:
...
- `react/src/style/icons/` holds PNG arrows (`green_arrow_up.png`, `green_thick_arrow_up.png`, `red_arrow_down.png`, `red_thick_arrow_down.png`, `winning-star.png`, `yellow_same_line.png`) — fuzzy on hi-DPI and can't inherit theme colors.

Replace both with Material Symbols so iconography matches MD3.
...
READ FIRST: docs/frontend-backlog.md section F7, react/src/Components/PageHeader.tsx (inline SVGs), then grep react/src for imports of the PNG files in react/src/style/icons/ to find every usage (standings movement arrows, winning star, etc.).
...
2. Build the Icon component with the needed glyphs: home, calendar_month, monitoring (or bar_chart), group, dark_mode, light_mode, arrow_back, arrow_upward, arrow_downward, remove (flat/no-change), and star. Size via prop, color via currentColor.
3. Replace the seven inline SVG functions in PageHeader.tsx with the Icon component; delete the dead SVG code.
4. Replace each PNG usage with the Icon component, colored via the success/danger tokens instead of baked-in green/red. CAUTION: check how the PNGs are referenced (img src vs CSS background) and keep layout dimensions identical.
5. Delete react/src/style/icons/ once nothing references it.

DONE WHEN:
- cd react && pnpm build passes and the icons/ folder is gone.
- Nav icons (mobile bottom bar), theme toggle, back button, standings movement arrows, and the winning star all render crisply at 1x and 2x DPR in both themes.
