...
5. Backend tests before frontend: mapper unit tests with trimmed fixtures (including missing-field cases), route tests asserting cache keys include year/category/rookie flag, mocked NHL clients throughout. cd api && pnpm test.
...
DONE WHEN:
- cd api && pnpm test passes with the new mapper/route/cache-key tests; cd react && npx tsc --noEmit and pnpm build pass.
