...
DONE WHEN:
- cd react && npx tsc --noEmit and pnpm build pass; if api/ was touched, cd api && pnpm test passes.
...
DONE WHEN:
- cd react && npx tsc --noEmit and pnpm build pass; cd api && pnpm test passes if api/ was touched.
...
5. Backend tests before frontend work: a mapper unit test with a trimmed fixture (skater + goalie + missing-fields cases) and a route test with a mocked NHL client asserting the cache key, copying the patterns in api/test/. Run cd api && pnpm test.
...
DONE WHEN:
- cd api && pnpm test passes (including the new tests); cd react && npx tsc --noEmit and pnpm build pass.
...
DONE WHEN:
- cd api && pnpm test passes (with the new mapper tests); cd react && npx tsc --noEmit and pnpm build pass.
...
DONE WHEN:
- cd api && pnpm test passes, including new validator unit tests (valid payload, each missing/wrong-typed field, non-JSON garbage).
...
5. Scripts: react/package.json gets "test": "vitest run" and "typecheck": "tsc --noEmit". Root package.json gets "verify" that runs, in order: api pnpm test, react typecheck, react test, react build — failing fast.
...
DONE WHEN:
- pnpm verify (root) passes end-to-end on a clean checkout with no network access beyond package installs.
...
DONE WHEN:
- Either: the measurement table is in docs/phase-2-progress.md section 2.14 with the decision to close/shrink — OR — the converted flows pass: cd api && pnpm test (new tests included), react verify pipeline green, and killing the database while the app runs degrades silently to the raw path (verified manually).
...
STEPS (pause after each):
1. pnpm add @anthropic-ai/sdk in api/. Create api/services/domain/anthropicClient.js (or fold into the 2.12 AI service if one exists): construct the client once at module scope, read the model from the same env var 2.12/C3 introduced, port the system prompt and max_tokens verbatim from hockey-ai.py.
...
DONE WHEN:
- cd api && pnpm test passes, including the new mocked-SDK tests.
