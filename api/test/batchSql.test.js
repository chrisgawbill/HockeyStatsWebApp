...
const {
	...
	buildValuesClause,
	batchUpsert,
} = require('../db/repositories/batchSql');
...
const teamSpec = {
	...
	onConflict: 'ON CONFLICT (team_id) DO UPDATE SET tri_code = EXCLUDED.tri_code',
...
test('dedupeByKey collapses same-key rows, keeping the last occurrence', () => {
	...
	assert.deepEqual(dedupeByKey(rows, (r) => String(r.id)), [
		{ id: 1, v: 'c' },
		{ id: 2, v: 'b' },
	]);
});
